...
const StoreDetails = () => {
  ...
  const fetchStores = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/admin/get-all-stores`);
      setStores(res.data);
    } catch (err) {
  ...
  const handleDelete = async (number) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/admin/delete-store/${number}`);
